...
import Ong.Models.PersonalSinRelaciones;
...
public class PersonalJpaController implements Serializable {
    ...
    public List<Ong.Models.PersonalSinRelaciones> findPersonalEntitiesSin() {
        return findPersonalEntitiesSin(true, -1, -1);
    }
    
    private List<Ong.Models.PersonalSinRelaciones> findPersonalEntitiesSin(boolean all, int maxResults, int firstResult) {
        EntityManager em = getEntityManager();
        try {
            CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
            cq.select(cq.from(PersonalSinRelaciones.class));
            Query q = em.createQuery(cq);
            if (!all) {
                q.setMaxResults(maxResults);
                q.setFirstResult(firstResult);
